  ...
  return (
    <div className="portfolio-stage relative overflow-hidden" style={themeStyle}>
      ...
      <main className="pt-24 md:pt-28">
        <section id="home" className="relative overflow-hidden py-20 transition-all duration-500 lg:py-28">
          <HeroDepthScene />
            ...
            <AnimateInView>
              <div>
                ...
                <h1 className="max-w-4xl font-[var(--font-heading)] text-5xl font-extrabold leading-none text-white sm:text-6xl lg:text-7xl">
                  <TypewriterText text={profile.headline} />
                </h1>
                ...
                <div className="mt-10 grid gap-6 sm:grid-cols-3">
                  {[
                    ...
                    <HoverCard key={item.label} className="rounded-3xl">
          ...
          <div className="section-shell">
            <AnimateInView>
              <SectionHeading
                ...
                description="Profile summary organized for interview use, blending business value with practical technical capability."
              />
            </AnimateInView>
            ...
            <div className="space-y-5">
              {experiences.map((item, index) => (
                <AnimateInView key={`${item.company}-${item.sortOrder}`} delay={index * 0.06}>
                  <HoverCard className="rounded-[2rem]">
                    <article className="glass depth-card rounded-[2rem] p-6 lg:p-8">
                      ...
                      {item.salary ? <p className="mt-2 text-sm text-slate-400">Salary: {item.salary}</p> : null}
                      <ul className="mt-5 space-y-3 text-slate-300">
            ...
            <AnimateInView>
              <div className="glass depth-card rounded-[2rem] p-6 lg:p-8">
                ...
                <div className="mt-6 space-y-5">
                  {educations.map((item) => (
                    <HoverCard key={`${item.school}-${item.sortOrder}`} className="rounded-3xl">
                      <div className="rounded-3xl border border-white/10 bg-white/5 p-5 transition duration-300 hover:border-white/20 hover:bg-white/[0.08]">
                        ...
                        <p className="text-sm text-slate-400">{[item.major, item.faculty].filter(Boolean).join(" · ")}</p>
              ...
              <div className="glass depth-card rounded-[2rem] p-6 lg:p-8">
                ...
                <div className="mt-6 space-y-5">
                  {profile.languages.map((language) => (
                    <HoverCard key={language.name} className="rounded-3xl">
                      <div className="rounded-3xl border border-white/10 bg-white/5 p-5 transition duration-300 hover:border-white/20 hover:bg-white/[0.08]">
                        <div className="text-lg font-semibold text-white">{language.name}</div>
                        <p className="mt-2 text-slate-300">
                          Speaking {language.speaking} · Reading {language.reading} · Writing {language.writing}
                        </p>
                      </div>
                    </HoverCard>
                  ))}
        ...
        <section id="contact" className="pb-20">
          <div className="section-shell">
            <AnimateInView>
              <div className="glass depth-card overflow-hidden rounded-[2rem] border border-white/10">
                <div className="grid gap-8 p-8 lg:grid-cols-[1.15fr_0.85fr] lg:items-center lg:p-10">
                  <div>
                    <span
                      className="inline-flex rounded-full px-3 py-2 text-xs font-semibold uppercase tracking-[0.24em]"
                      style={{ backgroundColor: `${profile.primaryColor}1a`, color: profile.primaryColor }}
                    >
                      Hire Me
                    </span>
                    <h2 className="mt-4 max-w-3xl font-[var(--font-heading)] text-3xl font-extrabold text-white lg:text-5xl">
                      Front-end polish, full stack support, and practical delivery for teams that need someone who can actually ship.
                    </h2>
                    <p className="mt-4 max-w-3xl text-lg leading-8 text-slate-300">
                      I focus on translating design into polished UI, connecting real data flow, and supporting product work from
                      front-end implementation through API and database-backed features. Open to {profile.availability.toLowerCase()},
                      based in {profile.location}, and available to start {profile.startWindow.toLowerCase()}.
                    </p>
                    ...
                    <div className="mt-6 flex flex-wrap gap-3">
                      ...
                      <a
                        ...
                        className="rounded-full border border-white/10 bg-white/5 px-5 py-3 font-semibold text-white transition duration-300 hover:-translate-y-1 hover:border-white/20 hover:bg-white/[0.08]"
                      >
                        {profile.phone}
                      </a>
                    </div>
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div className="rounded-3xl border border-white/10 bg-white/5 px-5 py-4 text-white">
                      <div className="text-xs uppercase tracking-[0.22em] text-slate-400">Role Focus</div>
                      <div className="mt-2 font-semibold">{profile.currentRole}</div>
                    </div>
                    <div className="rounded-3xl border border-white/10 bg-white/5 px-5 py-4 text-white">
                      <div className="text-xs uppercase tracking-[0.22em] text-slate-400">Resume Focus</div>
                      <div className="mt-2 font-semibold">{profile.resumeFocus}</div>
                    </div>
                    <div className="rounded-3xl border border-white/10 bg-white/5 px-5 py-4 text-white">
                    ...
                    <div className="rounded-3xl border border-white/10 bg-white/5 px-5 py-4 text-white">
                      <div className="text-xs uppercase tracking-[0.22em] text-slate-400">Start Window</div>
                      <div className="mt-2 font-semibold">{profile.startWindow}</div>
                    </div>
                  </div>
                </div>

                <div className="border-t border-white/10 bg-slate-950/30 px-8 py-5 lg:px-10">
                  <div className="flex flex-col gap-3 text-sm text-slate-400 sm:flex-row sm:items-center sm:justify-between">
                    <p>
                      {profile.fullName} ({profile.nickname}) • {profile.resumeFocus}
                    </p>
